"""
BTC Delta Tracker v2 — Streamlit edition (single page, premium UI)
=================================================================
Aceeasi logica ca btc_delta_tracker_v2.py (Binance volum + order book +
Polymarket oracle), dar afisata intr-o singura pagina web frumoasa.

Rulare local:
    pip install -r requirements.txt
    streamlit run btc_delta_tracker_app.py

Deploy gratis (link pentru prieten, fara Python):
    1. push pe GitHub
    2. share.streamlit.io -> New app -> alegi repo + acest fisier
    3. primesti un link public
"""
...
def _start_binance_trade_ws(s):
    while s["running"]:
        try:
            ws = websocket.WebSocketApp(
                f"wss://stream.binance.com:9443/ws/{SYMBOL_BINANCE}@aggTrade",
...
def _start_binance_depth_ws(s):
    while s["running"]:
        try:
            ws = websocket.WebSocketApp(
                f"wss://stream.binance.com:9443/ws/{SYMBOL_BINANCE}@depth20@100ms",
...
def card(title, inner):
    st.markdown(f'<div class="card"><h4>{title}</h4>{inner}</div>', unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("Datele vin live prin WebSocket. Pagina se reincarca automat.")
    render(s)
    time.sleep(refresh)
    st.rerun()
